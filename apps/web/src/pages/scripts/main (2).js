...
const TOOLS = [
  ...
  { name: 'Password generator',         url: '/generators/password',       icon: '🔒', cat: 'Generator' },
  { name: 'Lorem Ipsum generator',      url: '/generators/lorem-ipsum',    icon: '📝', cat: 'Generator' },
  { name: 'EMI calculator',             url: '/utilities/emi-calculator',  icon: '🏦', cat: 'Utilities' },
  { name: 'GST calculator',             url: '/utilities/gst-calculator',  icon: '📊', cat: 'Utilities' },
  { name: 'SIP calculator',             url: '/utilities/sip-calculator',  icon: '📈', cat: 'Utilities' },
];
...
function renderRecentTools() {
  ...
  try {
    ...
    container.innerHTML =
      '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:16px 28px;border-bottom:1px solid var(--border);background:var(--white)">' +
      '<span style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.7px;color:var(--slate-l);flex-shrink:0;margin-right:4px">Recently used</span>' +
      tools.map(function(t) {
        return '<a href="' + t.url + '" style="display:inline-flex;align-items:center;gap:7px;padding:6px 12px;border-radius:8px;border:1px solid var(--border);background:var(--white);font-size:13px;font-weight:500;color:var(--slate-m);transition:border-color .15s,color .15s" ' +
          'onmouseover="this.style.borderColor='var(--orange)';this.style.color='var(--orange-d)'" ' +
          'onmouseout="this.style.borderColor='var(--border)';this.style.color='var(--slate-m)'">' +
          '<span style="font-size:15px;line-height:1">' + t.icon + '</span>' +
          t.name + '</a>';
      }).join('') +
      '</div>';
